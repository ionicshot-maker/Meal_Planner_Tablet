...
import { DEFAULT_CATEGORIES, isMacrosIncomplete, normalizedToIngredient, lookupBarcodeProduct } from '@/utils/barcodeLookup'
  ...
  async function lookupBarcode(code: string) {
    ...
    try {
      ...
      if (result.status === 'error') {
        setError(`Lookup failed: ${result.errorMessage}`)
        return
      }
      if (!result.product) {
      ...
      const draft = normalizedToIngredient(result.product, categories)

      if (isMacrosIncomplete(result.product.macros) && settings.geminiApiKey) {
        const enriched = await tryGeminiEnrich(draft, result.product.product_name, result.product.brands)
        if (enriched) {
