...
- **Dev Tools → Barcode Duplicate Finder**: each candidate pair gets a "Keep" radio (defaults to whichever entry has the *longer* raw barcode string — directly matches the leading-zero bug's own signature, since the intact 13-digit code is the more complete/original one), a **Merge into "…"** button per non-keep entry, a per-entry **Delete** (with the reference-check warning), and a group-level **Skip this pair** (session-only dismissal — reappears on a later scan, nothing is persisted, so a skip can never silently hide a real duplicate forever).
- **Ingredients page → "Merge…" button** on every ingredient card: opens `MergeIngredientModal`, search for the other ingredient (reuses the existing `IngredientPicker`), then a side-by-side compare with a Keep radio (defaults to the card you started from) showing both ingredients' reference counts before you confirm. For duplicates that have nothing to do with barcodes at all — manually-typed double-entries, two separate imports of the same name.
...
**Verification status for the merge feature:** `npm run build` (tsc + vite) passes with the new code. The core repointing logic was verified against realistic mock data covering recipes (with and without a pinned variant, plus an unrelated and an unlinked row), all three grocery-list item buckets, macro logs, and the self-merge/missing-ingredient guard clauses — 16/16 checks passed. This was **not** run against a real IndexedDB instance (no `fake-indexeddb`/test runner is installed in this project) and **not** clicked through live in a browser (no connected Chrome extension or signed-in owner session available in the session that built this) — the actual `db.put`/`getDB()` calls follow the same established pattern as existing, working code elsewhere in this file (e.g. `applyIngredientBatch`), but weren't independently exercised end-to-end.
...
**Staged/atomic approval workflow — real, with one gap.** Nothing writes to the database while lines are parsed or matched — `ReceiptLineReview.tsx` holds every line in local state, independently editable and independently validated (servings > 0, unit price > 0, a name if creating new, a resolved match/Create-New choice, a resolved price decision). Saving only happens on **Save All**. All *ingredient* writes for the batch (`updatePrice` / `addVariant` / `createIngredient`) go through `applyIngredientBatch()` as one IndexedDB transaction — per the code's own comment, "either all of it lands or none of it does." Household-item rows are **not** part of that transaction; they're saved individually in a plain loop right after, so a failure partway through that loop could in principle leave the ingredient batch committed while only some household items saved. The receipt is only marked processed (see below) after both steps succeed, and already-saved rows are skipped on a retry, so a partial failure can be re-submitted without double-writing what already landed.
