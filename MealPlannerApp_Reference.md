...
**Barcode Duplicate Finder (added 2026-07-24):** a one-time cleanup companion to the bug above. `src/utils/barcodeDuplicateScan.ts`'s `findLeadingZeroBarcodeDupes()` loads all (non-archived) ingredients, strips leading zeros from every variant barcode, and groups by the stripped value — surfacing any group where more than one *distinct raw string* maps to the same stripped value (a group where every entry already shares the identical raw string is a plain, ordinary duplicate, not this bug, and is deliberately not flagged). Runs entirely client-side against the local IndexedDB data, on a manual "Scan" button click — nothing runs automatically. Results are grouped by matching barcode with a direct `/ingredients?edit=<id>` link per entry (the same deep-link pattern the barcode-scan-to-existing-ingredient flow already uses) so the user can open each candidate and decide manually whether to merge or delete — **matches the `_REVIEW.md` philosophy the converter itself uses: surface for human judgment, never silently auto-fix.** There is no merge feature in the app today (checked — none exists), so resolving a confirmed pair is a manual edit/delete in Ingredients. Verified: the grouping/normalization logic was tested directly against realistic mock data (two real leading-zero pairs, plus deliberate control cases — an identical-raw-string pair that should *not* flag, a unique barcode, an all-zero barcode, a missing barcode) and correctly found exactly the two real pairs while excluding every control case; `npm run build` passes with the new code included. **Not live-verified through the actual Dev Tools UI** — no connected browser or household-owner test credentials were available in the session that built this (same limitation noted for the access-control fix above).
